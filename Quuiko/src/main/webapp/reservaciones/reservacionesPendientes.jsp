...
<s:url namespace="/reservaciones" action="showPendingReservationsPag" var="urlSearch"/>
<s:url namespace="/reservaciones" action="authReservation" var="urlAutorizar"/>
<s:url namespace="/reservaciones" action="rejectReservation" var="urlRechazar"/>
...
<!-- 	<link type="text/css" href="css/mobile/style.css" rel="stylesheet" media="all" /> -->
	...
	<script type="text/javascript">
		...
		function consultarReservaciones(paginaActual){
			$.ajax({
				...
				data:{
					...
					'filtroReservacion.fechaReservacionFin':$('#fechaFin').val(),
		...
		function renderearReservacion(reservacion){
			...
			if(autorizacionEnviada==0 && (estatus=='PEND' || estatus=='PENDIENTE')){
			...
			html+='				<span class="glyphicon glyphicon-ok iconoProducto" onclick="autorizarReservacion('+id+');" title="Autorizar"></span> ';
			html+='				<span class="glyphicon glyphicon-remove iconoProducto" onclick="rechazarReservacion('+id+');" title="Rechazar"></span> ';
			html+='			</div> ';
			html+='			<h2>'+nombre+'</h2> ';
		...
		function autorizarReservacion(id){
			$.ajax({
				dataType:"json",
		...
		function rechazarReservacion(id){
			$.ajax({
				dataType:"json",
				data:{'idReservacion':id},
				url:"${urlRechazar}",
...
<s:form namespace="/productos" method="POST" id="forma" cssClass="form-horizontal" enctype="multipart/form-data">
	...
	<fieldset>
		  <legend>Reservaciones pendientes por autorizar</legend>
		  ...
		  <div class="form-group">
			<label class="col-lg-2 col-md-3 col-sm-5 col-xs-12 control-label">Fecha de Reservación de inicio:</label>
		    <div class="col-lg-2 col-md-3 col-sm-6 col-xs-12">
		      <input id="fechaInicio" type="text" class="form-control" placeholder="ej:(21/08/2014)" data-inputmask="'alias': 'date','mask':'d/m/y'"/>
		    </div>
		    
		    <label  class="col-lg-2 col-md-3 col-sm-5 col-xs-12 control-label">Fecha de Reservación de fin:</label>
		    <div class="col-lg-2 col-md-3 col-sm-6 col-xs-12">
		      <input id="fechaFin" type="text" class="form-control" placeholder="ej:(21/08/2014)" data-inputmask="'alias': 'date','mask':'d/m/y'"/>
		    </div>
		 </div>
			  ...
			  <li class="list-group-item itemProducto">
				    <div class="infoProducto">
				    	<div style="display:inline-block;float:right;">
				    		<s:if test="r.autorizacionEnviada==0 && ( r.estatusStr=='PEND' || r.estatusStr=='PENDIENTE')">
				    		...
				    		<span class="glyphicon glyphicon-ok iconoProducto" onclick="autorizarReservacion(${r.id});" title="Autorizar"></span>
				    		<span class="glyphicon glyphicon-remove iconoProducto" onclick="rechazarReservacion(${r.id});" title="Rechazar"></span>
				    	</div>
